...
const NumberGuessing = ({ currentUser, stakeAmount, opponentId, gameId, onGameEnd }: NumberGuessingProps) => {
  ...
  const handleGuessSubmit = async () => {
    ...
    if (isNaN(guessNumber) || guessNumber < 1 || guessNumber > 100) {
      toast({ title: "Invalid Guess", description: "Please enter a number between 1 and 100", variant: "destructive" });
      return;
    }
    ...
    const updatedGuesses = { ...currentGuesses, [user.id]: guessNumber };
    ...
    if (bothPlayersGuessed) {
      ...
      if (player1Distance < player2Distance) winnerId = game.player1_id;
      else if (player2Distance < player1Distance) winnerId = game.player2_id;
      ...
      await updateGameState({
        ...
        roundData: {
          ...
          results: { targetNumber, player1Guess, player2Guess, player1Distance, player2Distance, winner: winnerId }
        },
        winner: winnerId
      }, 'completed');
      ...
      if (winnerId === user.id) {
        toast({ title: "Congratulations!", description: `You won ${stakeAmount * 2} Gold! Your guess was closest to ${targetNumber}.` });
      } else if (winnerId) {
        toast({ title: "Game Over!", description: `You lost ${stakeAmount} Gold. The target was ${targetNumber}.`, variant: "destructive" });
      } else {
        toast({ title: "It's a Tie!", description: `Both players were equally close to ${targetNumber}. Stakes refunded.` });
      }
    } else {
      await updateGameState({
        roundData: { ...gameState.roundData, guesses: updatedGuesses }
      });
    }
  };
  ...
  const resetGame = async () => {
    ...
    await updateGameState({
      ...
      roundData: { guesses: {}, targetNumber: Math.floor(Math.random() * 100) + 1 },
      winner: null
    }, 'in_progress');
    setGuess("");
    setMyGuess(null);
              ...
              <div className="max-w-md mx-auto mb-6">
                <div className="flex space-x-4">
                  <Input type="number" min="1" max="100" value={guess} onChange={(e) => setGuess(e.target.value)}
                    placeholder="Enter your guess (1-100)" className="bg-gray-700 border-gray-600 text-white placeholder-gray-400" />
                  <Button onClick={handleGuessSubmit} className="bg-blue-600 hover:bg-blue-700 text-white">Submit</Button>
                </div>
              </div>
            )}
